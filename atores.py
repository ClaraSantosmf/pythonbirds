    ...
    def colidir(self, outro_ator, intervalo=1):
        ...
        if self.status == ATIVO and outro_ator.status == ATIVO:
            ...
            if delta_x <= intervalo and delta_y <= intervalo:
                self.status = outro_ator.status = DESTRUIDO #dessa forma a atribuição consegue ir para ambos os objetos
...
class Passaro(Ator):
    ...
    def calcular_posicao(self, tempo):
        ...
        if self.esta_voando():
            ...
            self._calcular_posicao_vertical(delta_t) #criado um método protegido auxiliar para melhor organizar o code
            self._calcular_posicao_horizontal(delta_t) #criado um método protegido auxiliar para melhor organizar o code
    ...
    def _calcular_posicao_vertical(self, delta_t): #implementando calculo exposto na documentação.
